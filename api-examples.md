...
- **类型**: `LevelType`（枚举）
- **描述**: 定义关卡的类型，如主线关卡、支线关卡、生存模式等
- **示例值**: `MainStory`, `Survival`, `MiniGame`
...
- **类型**: `string`
- **描述**: 关卡的英文名称
- **示例值**: `"Grass Level"`, `"Roof Level"`

### LevelPlantType

- **类型**: `PlantType`（枚举）
- **描述**: 关卡允许使用的植物类型
- **示例值**: `Basic`, `Advanced`, `All`
...
- **类型**: `int[,]`
- **描述**: 关卡地图的道路类型矩阵
- **示例**: `new int[,] { { 1, 2 }, { 3, 4 } }`，其中每个数字代表不同的道路类型
...
- 关卡图标(LevelSprite)建议使用256x256像素的图片，以获得最佳显示效果
- MapRoadTypes矩阵的维度应与关卡场景的地图大小匹配
- 避免在EnterAction中执行过多耗时操作，以免影响游戏体验
- 确保ScenePrefab包含必要的组件和脚本，如地图管理器、波次控制器等
